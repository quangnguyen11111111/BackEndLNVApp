    ...
    return new Promise(async(resolve, reject) => {
        try {
            ...
            if(!checkAccount){
                ...
                let checkPassword =  bcrypt.compareSync(dataBody.userPassword, user.userPassword);
                if (!checkPassword) {
                    data.errCode=1
                    ...
                    await db.users.update({ refreshToken },{ where: { userID: user.userID },raw:true });
                    data.errCode=0
    ...
    return new Promise(async(resolve, reject) => {
       try {
        ...
        try {
            ...
            let user = await db.users.findOne({where:{userID:decoded.userID}});
        if (!user || user.refreshToken !== dataBody) {
            data.errCode=4,
            data.message="Token không hợp lệ"
          }else{ const newAccessToken =  generateAccessToken(user)
            data.errCode=0,
